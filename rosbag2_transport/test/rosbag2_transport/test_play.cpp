...
TEST_F(RosBag2PlayTestFixture, recorded_messages_are_played_for_all_topics_multi)
{
  ...
  constexpr std::size_t num_bags = 3u;
  ...
  for (std::size_t i = 0u; i < num_bags; i++) {
    auto prepared_mock_reader = std::make_unique<MockSequentialReader>();
  ...
  rcutils_time_point_value_t last_timetamp = \
    std::numeric_limits<rcutils_time_point_value_t>::lowest();
